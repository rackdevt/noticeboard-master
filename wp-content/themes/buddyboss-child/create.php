...
<!--<?php echo '$_POST["group-name"]'; ?>

<?php
echo $_GET['group-name'];
echo "<strong>Group ".$_GET['group-name']. " is being registered on the GNB...</strong><br/>";
	...
	<div id="main-column">
			...
			<div class="item-body" id="group-create-body">
				...
				<?php if ( bp_is_group_creation_step( 'group-details' ) ) : ?>
					...
					<input type="text" name="group-name" id="group-name" value="<?php echo $_GET['group-name']; ?>" />
				...
				<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
					...
					<div class="radio">
						<label><input type="radio" name="group-status" value="public"<?php if ( 'public' == bp_get_new_group_status() || !bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> />
							...
							<ul>
								<li><?php _e( 'Any registered member can join this group', 'buddypress' ); ?></li>
								<li><?php _e( 'This group will be listed in the groups directory and in search results', 'buddypress' ); ?></li>
								<li><?php _e( 'Group content and activity will be visible to any logged-in GNB member', 'buddypress' ); ?></li>
							</ul>
						</label>
						...
						<label><input type="radio" name="group-status" value="private"<?php if ( 'private' == bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> />
							...
							<ul>
								<li><?php _e( 'Only logged-in GNB members who request membership and are accepted can join the group', 'buddypress' ); ?></li>
								<li><?php _e( 'This group will be listed in the groups directory and in search results', 'buddypress' ); ?></li>
						...
						<label><input type="radio" name="group-status" value="hidden"<?php if ( 'hidden' == bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> />
							<strong><?php _e('This is a closed group', 'buddypress'); ?></strong>
							<ul>
								<li><?php _e( 'Only GNB members who are invited can join the group', 'buddypress' ); ?></li>
								<li><?php _e( 'This group will not be listed in the groups directory or search results', 'buddypress' ); ?></li>
								<li><?php _e( 'Group content and activity will only be visible to members of the group', 'buddypress' ); ?></li>
								<li><?php _e( 'Group is suitable for children/vulnerable people', 'buddypress' ); ?></li>
							</ul>
						</label>
					</div>
					...
					<div class="radio">
						...
						<label>
							...
							<strong><?php _e( 'Group administrators and moderators only', 'buddypress' ); ?></strong>
						</label>

						<label>
							<input type="radio" name="group-invite-status" value="admins"<?php bp_group_show_invite_status_setting( 'admins' ); ?> />
							<strong><?php _e( 'Group administators only', 'buddypress' ); ?></strong>
						</label>
					</div>
				...
				<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
					...
					<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
						...
						<div class="main-column">
							<p><?php _e( "Upload an image to use as an avatar for this group. Please ensure image size is 150 pixels wide by 150 pixels high. The image will be shown on the main group page, and in search results.", 'buddypress' ); ?></p>
