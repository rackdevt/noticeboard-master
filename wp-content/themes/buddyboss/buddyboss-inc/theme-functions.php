...
function buddyboss_setup()
{
	...
	// Adds wp_nav_menu() in two locations with BuddyPress deactivated.
		register_nav_menus( array(
			...
			'generic-menu' => __( 'PaM Menu', 'buddyboss' ),
			'generic-footer-menu' => __( 'PaM Footer Menu', 'buddyboss' ),
		) );
...
function buddyboss_widgets_init() {
	...
	// Area 17, located in the Humanity Fund - Poundamonth area. Only appears if widgets are added.
	register_sidebar( array(
			'name'          => 'Humanity Fund &rarr; Right Sidebar',
			'id'          	=> 'pam-sidebar',
			'description'   => 'The Humanity Fund-PaM Right widget area. Right column only appears if widgets are added.',
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		    'after_widget'  => '</aside>',
		    'before_title'  => '<h4 class="widgettitle">',
		    'after_title'   => '</h4>'
