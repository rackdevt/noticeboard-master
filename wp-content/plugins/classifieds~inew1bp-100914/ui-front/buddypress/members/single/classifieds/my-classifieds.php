				...
				<div class="cf-pad">
					...
					<div class="cf-info">
						...
						<div>
								...
								<span><?php _e( '  Type: ', $this->text_domain ); ?><?php echo do_shortcode('[ct id="ct_List_Type_checkbox_537b" property="value"]'); ?></span> <br />
	
								<span><?php _e( '  Date Posted: ', $this->text_domain ); ?><?php echo do_shortcode('[ct id="ct_Post_Date_datepicker_2ddf" property="value"]'); ?> <br />

								<span><?php _e( '  Expires on: ', $this->text_domain ); ?><?php echo $this->get_expiration_date( get_the_ID() ); ?></span><br />
								
								<span><?php _e( '  Status: ', $this->text_domain ); ?><?php echo do_shortcode('[ct id="ct_Item_Avail_text_6e7b" property="value"]'); ?></span> 
					...
					<form action="#" method="post" id="action-form-<?php the_ID(); ?>" class="action-form">
						...
						if(current_user_can('edit_classified', get_the_ID())){
							echo do_shortcode('[cf_edit_classified_btn text="' . __('Update Availability Status', $this->text_domain) . '" view="always" post="' . get_the_ID() . '"]');
						}
						?>
					...
					<form action="#" method="post" id="confirm-form-<?php the_ID(); ?>" class="confirm-form">
						...
						<select name="duration">
							...
							if($this->use_credits):
							?>
							<option value="<?php echo $field_option; ?>"><?php  echo sprintf(__('%s', $this->text_domain), $field_option, round($field_option + 0) * $cf_payments['credits_per_week']); ?></option>
							<?php else: ?>
