...
Description: Gives users the ability to 'like' content across your BuddyPress enabled site. Modified by @webdwall to show both like and unlike buttons simultaneously
 ...
 * bp_like_is_unliked()
 *
 * Checks to see whether the user has unliked a given item.
 *
 */
function bp_like_is_unliked( $item_id = '', $type = '', $user_id = '' ) {
	global $bp;
	...
	if ( $type == 'activity' )
		$user_likes = get_user_meta( $user_id, 'bp_unliked_activities', true );
	
	if ( $type == 'blogpost' )
		$user_likes = get_user_meta( $user_id, 'bp_unliked_blogposts', true );
...
function bp_like_add_user_like( $item_id = '', $type = 'activity' ) {
	...
	if ( $type == 'activity' ) :
		...
		/* remove from user's unliked activities. */
		$user_unlikes = get_user_meta( $user_id, 'bp_unliked_activities', true );
		if(isset($user_unlikes[$item_id])){
			unset( $user_unlikes[$item_id] );
		...
		/* Remove from unlikes for this activity. */
		$users_who_unlike = bp_activity_get_meta( $item_id, 'unliked_count', true );
		if(isset($users_who_unlike[$user_id])){
			unset($users_who_unlike[$user_id]);
			bp_activity_update_meta( $item_id, 'unliked_count', $users_who_unlike );
		}
	
		$liked_count = count( $users_who_like );
		$unliked_count = count( $users_who_unlike );
	...
	elseif ( $type == 'blogpost' ) :
		...
		/* remove from user's unliked blog posts. */
		$user_unlikes = get_user_meta( $user_id, 'bp_unliked_blogposts', true );
		if(isset($user_unlikes[$item_id])){
			unset( $user_unlikes[$item_id] );
			update_user_meta( $user_id, 'bp_unliked_blogposts', $user_likes );
		}
		...
		/* Remove from total dislikes for this blog post */
	...
	if ($liked_count)
		$response['likecount'] = $liked_count;
	if ($unliked_count)
		$response['unlikecount'] = $unliked_count;
...
function bp_like_remove_user_like( $item_id = '', $type = 'activity') {
	...
	if ( $type == 'activity' ) :
		...
		$users_who_like = bp_activity_get_meta( $item_id, 'liked_count', true );
		if(isset($users_who_like[$user_id])){
...
function bp_like_button( $id = '', $type = '' ) {
	wp_enqueue_style( 'bp-like-css', WP_PLUGIN_URL .'/buddypress-like/_inc/buddypress-like.css' );
	$users_who_like = 0;
	$liked_count = 0;
	$unliked_count = 0;
	...
	if ( $type == 'activity' ) :
		...
		if ( is_user_logged_in() && $activity_type !== 'activity_liked' && $activity_type !== 'activity_unliked' ) :
			...
			if ( bp_activity_get_meta( bp_get_activity_id(), 'unliked_count', true )) {
				$users_who_unlike = array_keys( bp_activity_get_meta( bp_get_activity_id(), 'unliked_count', true ) );
				$unliked_count = count( $users_who_unlike );
			}
			/*
			...
			*/
			?>
			<?php if (!bp_like_is_liked( bp_get_activity_id(), 'activity' ) ) : ?>
				<a href="#" class="like" id="like-activity-<?php bp_activity_id(); ?>" 
					title="<?php echo bp_like_get_text( 'like_this_item' ); ?>">
					<?php echo bp_like_get_text( 'like' ); if ( $liked_count ) echo ' (' . $liked_count . ')'; ?></a>
			<?php else : ?>
				<a class="bplike-prev-like" id="apl_like-activity-<?php bp_activity_id(); ?>">
					<?php
					$other_like_count = $liked_count -1 ;
					if($other_like_count < 0){$other_like_count =0;}
					?>
					<?php echo "<span>" . bp_like_get_text( 'like' ) ."</span>: You+($other_like_count) other(s)";?>
				</a>
			<?php endif; ?>
			<?php if (!bp_like_is_unliked( bp_get_activity_id(), 'activity' ) ) : ?>
				<a href="#" class="unlike" id="unlike-activity-<?php bp_activity_id(); ?>" 
					title="<?php echo bp_like_get_text( 'unlike_this_item' ); ?>">
					<?php echo bp_like_get_text( 'unlike' ); if ( $unliked_count ) echo ' (' . $unliked_count . ')'; ?></a>
			<?php else:?>
				<a class="bplike-prev-unlike" id="apul_like-activity-<?php bp_activity_id(); ?>">
					<?php
					$other_unlike_count = $unliked_count -1 ;
					if($other_unlike_count < 0){$other_unlike_count =0;}
					?>
					<?php echo "<span>" . bp_like_get_text( 'unlike' ) ."</span>:You+($other_unlike_count) other(s)";?>
				</a>
			<?php endif; ?>
			<?php
			/*
	...
	elseif ( $type == 'blogpost' ) :
		...
		if ( is_user_logged_in() && get_post_meta( $id, 'unliked_count', true ) ) {
			$unliked_count = count( get_post_meta( $id, 'unliked_count', true ) );
		}
		/*
		...
		*/
		...
		<?php if ( !bp_like_is_liked( $id, 'blogpost' ) ) : ?>
			<div class="like-box">
				<a href="#" class="like_blogpost" id="like-blogpost-<?php echo $id; ?>" 
				title="<?php echo bp_like_get_text( 'like_this_item' ); ?>">
					<?php echo bp_like_get_text( 'like' ); if ( $liked_count ) echo ' (' . $liked_count . ')'; ?>
				</a>
			</div>
		<?php else: ?>
			<div class="like-box">
				<a href="#" class="bplike-prev-like">You like it</a>
			</div>
		<?php endif;?>
		<?php if ( !bp_like_is_unliked( $id, 'blogpost' ) ) : ?>
			<div class="like-box">
				<a href="#" class="unlike_blogpost" id="unlike-blogpost-<?php echo $id; ?>" 
					title="<?php echo bp_like_get_text( 'unlike_this_item' ); ?>">
					<?php echo bp_like_get_text( 'unlike' ); if ( $unliked_count ) echo ' (' . $unliked_count . ')'; ?>
				</a>
			</div>
		<?php else: ?>
			<div class="like-box">
				<a href="#" class="bplike-prev-unlike">You unlike it</a>
			</div>
		<?php endif;?>
		<?php
	endif;
