...
if ( ! class_exists( 'WPMUDEV_Chat' ) ) {
	class WPMUDEV_Chat {
		var $chat_current_version = '2.0.8.6';
		...
		function __construct() {
			...
			//add_shortcode('chat_archive', array(&$this, 'process_chat_archive_shortcode'));
		}
		...
		function check_upgrade() {
			...
			if ( ( empty( $this->_chat_plugin_settings['options_version'] ) )
				...
				if ( version_compare( $this->_chat_plugin_settings['options_version'], '2.6' ) < 0 ) {
					...
					//echo "logs<pre>"; print_r($logs); echo "</pre>";
		...
		function load_configs() {
			...
			if ( ( isset( $this->chat_auth['type'] ) ) && ( $this->chat_auth['type'] == 'wordpress' ) ) {
				//log_chat_message(__FUNCTION__ .": type=wordpress");

				if ( is_user_logged_in() ) {
					//log_chat_message(__FUNCTION__ .": is_user_logged_in: yes");
			...
			//log_chat_message(__FUNCTION__ .": chat_auth (after)<pre>". print_r($this->chat_auth, true). "</pre>");
...
//		if (is_network_admin()) {
//
//			$this->_chat_options['widget'] = get_site_option( 'wpmudev-chat-widget', array() );
//			$this->_chat_options['widget'] = wp_parse_args( $this->_chat_options['widget'], $this->_chat_options_defaults['widget'] );
//		} else {
//			$this->_chat_options['widget'] = get_option( 'wpmudev-chat-widget', array() );
//			$this->_chat_options['widget'] = wp_parse_args( $this->_chat_options['dashboard'], $this->_chat_options_defaults['widget'] );
//		}
...
//		if (isset($new_config['log_display'])) {
//			if ($new_config['log_display'] == "enabled")
//				$new_config['log_display'] = 'enabled-list-below';
//
//			if (($new_config['log_display'] == "enabled-link-above") || ($new_config['log_display'] == "enabled-link-below"))
//				$new_config['log_display_hide_session'] = "hide";
//
//		} else {
//			$new_config['log_display'] = 'disabled';
//		}
			return $new_config;
		}
		...
		function set_option_defaults() {
			...
			//echo "site<pre>"; print_r($this->_chat_options_defaults['site']); echo "</pre>";
			//die();
			...
			$this->_chat_options_defaults['global'] = array(
				'session_poll_interval_messages' => defined( 'WPMUDEV_CHAT_GLOBAL_SESSION_POLL_INTERVAL_MESSAGES' ) ? WPMUDEV_CHAT_GLOBAL_SESSION_POLL_INTERVAL_MESSAGES : '1',
			...
			if ( is_user_logged_in() ) {
				...
				//echo "user_meta_option<pre>"; print_r($user_meta_option); echo "</pre>";
				//die();
				...
				//echo "user_meta<pre>"; print_r($this->_chat_options_defaults['user_meta']); echo "</pre>";
				//die();
		...
		function wp_head() {
			...
			if ( ! empty( $this->_registered_styles ) ) {
				//echo "_registered_styles<pre>"; print_r($this->_registered_styles); echo "</pre>";
				//die();
...
//			//echo "chat_network_site_box_container: site_content[". $site_content ."]<br />";
//			//die();
//		}
//
//		if (empty($site_content)) {
//			$site_content .= $this->chat_site_box_container();
//			//echo "chat_site_box_container: site_content[". $site_content ."]<br />";
//		}
//
//		if ($this->get_option('box_shadow_show', 'site') == "enabled") {
		...
		function admin_notices() {
			...
			if ( ( isset( $this->_admin_notice_messages_key ) ) && ( ! empty( $this->_admin_notice_messages_key ) ) ) {
				//echo "_admin_notice_messages_key[". $this->_admin_notice_messages_key ."]<br />";
		...
		function network_admin_menu() {
			...
			/*
		$this->_pagehooks['chat_settings_panel_network_widget'] = add_submenu_page('chat_settings_panel_network_site',
					_x('Settings Widget', 'page label', $this->translation_domain),
					_x('Settings Widget', 'menu label', $this->translation_domain),
					'manage_network_options',
					'chat_settings_panel_network_widget',
					array(&$this->_admin_panels, 'chat_settings_panel_network_widget')
		);
		*/
		...
		function on_load_panels() {
			...
			$this->_admin_notice_messages['success-settings'] = __( "Settings have been update.", $this->translation_domain );
				...
				if ( ( isset( $_GET['maction'] ) ) && ( isset( $_GET['_wpnonce'] ) ) && ( wp_verify_nonce( $_GET['_wpnonce'], 'chat-message-item' ) ) ) {
					...
					if ( $maction == "delete" ) {
						...
						if ( isset( $_GET['lid'] ) ) {
							...
							//echo "log_item<pre>"; print_r($log_item); echo "</pre>";
							if ( ( ! empty( $log_item ) )
								...
								$chat_session_transient = get_transient( $transient_key );
								//$chat_session_transient = get_option($transient_key);
								//echo "chat_session_transient<pre>"; print_r($chat_session_transient); echo "</pre>";
		...
		function process_panel_actions() {
			...
			if ( isset( $_POST['chat'] ) ) {
				...
				if ( isset( $chat_settings['section'] ) ) {
					...
					//echo "section: ". $chat_sections ." chat_settings<pre>"; print_r($chat_settings); echo "</pre>";
					//die();
		...
		function chat_edit_user_profile( $user = '' ) {
			...
			//echo "user_meta defaults<pre>"; print_r($this->_chat_options_defaults['user_meta']); echo "</pre>";
			//echo "widget<pre>"; print_r($this->_chat_options_defaults['widget']); echo "</pre>";
			//echo "widget<pre>"; print_r($this->_chat_options['dashboard']); echo "</pre>";
		...
		function chat_save_user_profile( $user_id = '' ) {
			...
			//echo "wpmudev_chat_user_settings<pre>"; print_r($_POST['wpmudev_chat_user_settings']); echo "</pre>";
			//die();
		...
		function process_chat_shortcode( $atts ) {
			...
			foreach ( $control_arrays as $control_item ) {
				//echo "1. ". $control_item ."<pre>"; print_r($atts[$control_item]); echo "</pre>";
				if ( ( ! isset( $atts[ $control_item ] ) ) || ( empty( $atts[ $control_item ] ) ) ) {
					//$atts[$control_item] = array();
					continue;
			...
			if ( ( isset( $atts['session_type'] ) ) && ( $atts['session_type'] == 'site' ) ) {
				...
				//echo "page<pre>"; print_r($this->_chat_options['page']); echo "</pre>";
				//die();
			...
			//echo "2. ". $control_item ."<pre>"; print_r($atts[$control_item]); echo "</pre>";
			/*
		foreach($control_arrays as $control_item) {

			if (is_array($chat_session[$control_item])) {
				if (count($wp_roles)) {
					foreach ($wp_roles->roles as $role_slug => $role) {
						if (isset($role['capabilities']['level_10'])) {
							if (in_array($role_slug, $chat_session[$control_item]) === false) {
								$chat_Session[$control_item][] = $role_slug;
							}
						}
					}
				}
		...
		*/
			...
			//if (isset($atts['update_transient'])) {
			//	$chat_session['update_transient'] = $atts['update_transient'];
			//	echo "isset<br />";
			//} else {
			//	$chat_session['update_transient'] = true;
			//	echo "!isset<br />";
			//}
			//echo "update_transient[". $atts['update_transient'] ."][". $chat_session['update_transient'] ."]<br />";

			$chat_session['last_row_id'] = "__EMPTY__";
			//$this->chat_session_get_last_row_id($chat_session);
			$this->chat_session_update_message_rows_deleted( $chat_session );

			// Set the active users. Helps when adding/removing users after the page loads.
			//$chat_session['users'] = $this->chat_session_get_active_users($chat_session);
			...
			if ( $chat_session['session_type'] != "private" ) {
				//echo "moderator_roles<pre>"; print_r($chat_session['moderator_roles']); echo "</pre>";
				//$chat_session['moderator_roles'] = '';
				...
				if ( ! isset( $this->chat_auth['type'] ) ) {
					...
					if ( $this->chat_auth['type'] == "wordpress" ) {
						...
						if ( ! is_user_logged_in() ) {
							// We reset the chat_auth to be invalid.
							//$this->chat_auth 			= array();
							//$this->chat_auth['type'] 	= '';
							...
							//echo "bp<pre>"; print_r($bp->groups->current_group); echo "</pre>";
							//die();
								...
								if ( ( is_multisite() ) && ( ! is_super_admin() ) ) {
									...
									if ( isset( $user_blogs[ $blog_id ] ) ) {
										...
										if ( ! array_intersect( $chat_session['login_options'], $current_user->roles ) ) {
											//log_chat_message(__FUNCTION__ .": chat_sessions<pre>". print_r($chat_session, true). "<pre>");;
											//log_chat_message(__FUNCTION__ .": login_options<pre>". print_r($chat_session['login_options'], true). "<pre>");;
											//log_chat_message(__FUNCTION__ .": ". __LINE__ .": here");;
			...
			// Convert the previous
//		if ($chat_session['log_display'] == "enabled")
//			$chat_session['log_display'] = 'enabled-list-below';
//
...
//		}
		...
		function process_chat_archive_shortcode( $atts ) {
			...
			//$chat_session['session_type'] = 'page';
			//echo "in chat_archive chat_session<pre>"; print_r($chat_session); echo "</pre>";
...
//		}
			...
			if ( ( $chat_session['box_emoticons'] == "enabled" ) || ( $chat_session['box_sound'] == "enabled" ) || ( intval( $chat_session['row_message_input_length'] ) > 0 ) ) {
				...
				*/
				...
				//echo "chat_session<pre>"; print_r($chat_session); echo "</pre>";
				//die();
				//echo "length[". $chat_session['row_message_input_length'] ."]<br />";
				if ( intval( $chat_session['row_message_input_length'] ) > 0 ) {
...
//					if ($this->get_option('box_shadow_show', 'network-site') == "enabled") {
				...
				//							wpmudev_chat_check_size_qualifier($this->get_option('box_shadow_h', 'network-site'), array('px')) .' '.
				//							wpmudev_chat_check_size_qualifier($this->get_option('box_shadow_blur', 'network-site'), array('px')) .' '.
				//							wpmudev_chat_check_size_qualifier($this->get_option('box_shadow_spread', 'network-site'), array('px')) .' '.
				//							$this->get_option('box_shadow_color', 'site') .' }';
				//					$content .= '</style>';
				//				}
				//			}
		...
		function process_chat_actions( $return = 'no' ) {
			...
			switch ( $function ) {
				...
				case 'chat_init':
					...
					//echo "chat_sessions_meta<pre>"; print_r($this->chat_sessions_meta); echo "</pre>";
					...
					foreach ( $this->chat_sessions as $chat_id => $chat_session ) {
						...
						//	$chat_session_meta = $this->chat_sessions_meta[$chat_id];
						//	echo "chat_session_meta<pre>"; print_r($chat_session_meta); echo "</pre>";
						//
						//	$this->chat_session_verify_meta($chat_session, $chat_session_meta);
						//}
					}
				...
				case 'chat_message_send':
					...
					foreach ( $_POST['chat_messages'] as $chat_id => $chat_messages ) {
						...
						//error_log(__FUNCTION__ .": chat_id[". $chat_id ."] chat_message[". print_r($chat_messages, true) ."]");
				...
				case 'chat_messages_update':
					...
					//echo "timers<pre>"; print_r($timers); echo "</pre>";
					...
					if ( ( $this->using_popup_out_template == false )
						...
						if ( ( is_array( $invites ) ) && ( ! empty( $invites ) ) ) {
							...
							//foreach ($invites as $invite_session_id => $invite_session) {
							//	$this->chat_sessions[$invite_session_id] = $invite_session['session'];
							//}
						}

						//echo "invites<pre>"; print_r($reply_data['invites']); echo "</pre>";
						//echo "chat_sessions<pre>"; print_r($this->chat_sessions); echo "</pre>";
						//die();

						// Add the invites to the main sessions so we pick up the meta, users, etc.
						//if (is_array($reply_data['invites']))
						//	$this->chat_sessions = array_merge($this->chat_sessions, $reply_data['invites']);
					}
					...
					//die();
					foreach ( $this->chat_sessions as $chat_id => $chat_session ) {
						//echo "chat_id[". $chat_id ."] chat_session<pre>"; print_r($chat_session); echo "</pre>";
						...
						if ( ( ( isset( $timers['meta'] ) ) && ( $timers['meta'] == 1 ) ) || ( $chat_session['session_type'] == 'private' ) ) {
							...
							//echo "users-active<pre>"; print_r($reply_data['sessions'][$chat_id]['meta']['users-active']); echo "</pre>";
							$this->chat_session_users_update_polltime( $chat_session );
						}

						// We update the usermeta with the current tmestamp
						if ( ( ( isset( $timers['users'] ) ) && ( $timers['users'] == 1 ) ) || ( $chat_session['session_type'] == 'private' ) ) {
				...
				case 'chat_session_invite_private':
					...
					if ( ! empty( $invites ) ) {
						...
						//echo "empty invites<br />";
						//echo "invitation<pre>"; print_r($invitation); echo "</pre>";
...
//		$content 		.= $this->chat_session_module_wrap($chat_session, $content_tmp, 'wpmudev-chat-module-lists');
		...
		function chat_session_get_invites_new() {
			...
			if ( $this->user_meta['chat_user_status'] != 'available' ) {
			...
			if ( ! empty( $invite_chats ) ) {
				foreach ( $invite_chats as $invite_chat ) {
					...
					if ( ! isset( $this->chat_sessions[ $invite_chat['chat_id'] ] ) ) {
						...
						$content = $this->process_chat_shortcode( $atts );
						if ( ( ! empty( $content ) ) && ( isset( $this->chat_sessions[ $invite_chat['chat_id'] ] ) ) ) {
		...
		function wpmudev_chat_set_private_archive_status( $chat_session ) {
			...
			//log_chat_message(__FUNCTION__ .": sql_str". $sql_str);
			//log_chat_message(__FUNCTION__ .": user_meta". print_r($this->user_meta, true). "");

			$invite_chats = $wpdb->get_results( $sql_str );
			//error_log('invite_chats<pre>'. print_r($invite_chats, true) .'<pre>');
			if ( ! empty( $invite_chats ) ) {
				foreach ( $invite_chats as $invite_chat ) {
					$message = maybe_unserialize( $invite_chat->message );
					//error_log('message<pre>'. print_r($message, true) .'<pre>');
		...
		function wpmudev_chat_update_user_invite_status( $invite_status, $chat_id, $auth_hash = '', $invite_archived = '' ) {
			...
			//echo "invite_status[". $invite_status ."]<br />";
			//echo "auth_hash[". $auth_hash ."]<br />";
			//echo "invite_archived[". $invite_archived ."]<br />";
			...
			if ( ! empty( $invite_chat ) ) {
				...
				//echo "after: invite_chat<pre>"; print_r($invite_chat); echo "</pre>";
		...
		function chat_session_update_message_rows_deleted( $chat_session ) {
			...
			//echo "rows_tmp<pre>"; print_r($rows_tmp, true); echo "</pre>";
		...
		function chat_session_update_meta_log( $chat_session, $meta_data = array() ) {
			...
			//echo "chat_session<pre>"; print_r($chat_session); echo "</pre>";
			//echo "meta_data<pre>"; print_r($meta_data); echo "</pre>";
			...
			if ( ! isset( $meta_data['session-status'] ) ) {
				...
				if ( $chat_session['session_type'] == "private" ) {
					...
					//echo "chat_status<pre>"; print_r($chat_status); echo "</pre>";
					//echo "invite_chats<pre>"; print_r($invite_chats); echo "</pre>";
					//echo "chat_id[". $chat_session['id'] ."] session_type[". $chat_session['session_type']. "] chat_status[". $chat_status->archived ."]<br />";
					$meta_data['session-status'] = $chat_status->archived;
					//echo "meta_data<pre>"; print_r($meta_data); echo "</pre>";
				} else {
		...
		function chat_session_get_active_users( $chat_session ) {
			...
			if ( $chat_session['session_type'] == 'private' ) {
				//log_chat_message(__FUNCTION__ .": chat_session: private");
				//$sql_str = 	$wpdb->prepare("SELECT * FROM ". WPMUDEV_Chat::tablename('message') ." WHERE blog_id = %d AND chat_id=%s AND session_type=%s", 0, $chat_session['id'], 'invite');
				...
				//log_chat_message(__FUNCTION__ .": users: <pre>". print_r($users, true). "</pre>");
					...
					foreach ( $users as $user ) {
						...
						if ( ! empty( $user->last_polled ) ) {
							...
							if ( ( isset( $user->message ) ) && ( ! empty( $user->message ) ) ) {
								...
								//log_chat_message(__FUNCTION__ .": invitation: <pre>". print_r($invitation, true). "</pre>");
				...
				//echo "users_data<pre>"; print_r($users_data); echo "</pre>";
				//log_chat_message(__FUNCTION__ .": users_data: <pre>". print_r($users_data, true). "</pre>");
			...
			last_polled < TIMESTAMPADD(SECOND, -" . $chat_delete_threshold . ", NOW());", $chat_session['id'] );
				...
				//$sql_str = "SELECT * FROM ". WPMUDEV_Chat::tablename('users') ." ORDER BY name ASC";
		...
		function chat_session_send_message( $message, $chat_session ) {
			...
			$ret = $wpdb->query( $sql_str );
			if ( ( isset( $wpdb->insert_id ) ) && ( $wpdb->insert_id > 0 ) ) {
				//log_chat_message(__FUNCTION__ .": message: [". $message ."][". $wpdb->insert_id ."]");
				//$this->chat_session_set_meta($chat_session, 'last_row_timestamp', $time_stamp_seconds);
		...
		function chat_session_archive_messages( $chat_session ) {
			...
			if ( ( $chat_messages ) && ( count( $chat_messages ) ) ) {
				...
				//echo "chat_log_id[". $chat_log_id ."]<br />";
				//die();
		...
		function chat_session_get_log_row_id( $chat_session ) {
			...
			//echo "sql_str[". $sql_str ."]<br />";
			$chat_log = $wpdb->get_row( $sql_str );
			//echo "chat_log<pre>"; print_r($chat_log); echo "</pre>";
		...
		function chat_session_show_via_logs( $chat_session ) {
			...
			if ( is_admin() ) {
				...
				if ( isset( $_POST['wpmudev-chat-settings-request-uri'] ) ) {
					...
					//echo "REFERER_args<pre>"; print_r($REFERER_args); echo "</pre>";
					//if (empty($REFERER_args)) {
