...
//	init_users: true,
    ...
    chat_session_message_update: function () {
        ...
        if (wpmudev_chat.settings['sessions'] != undefined) {
            for (var chat_id in wpmudev_chat.settings['sessions']) {
                ...
                if (jQuery('div#wpmudev-chat-box-' + chat_id + ' div.wpmudev-chat-module-messages-list').length) {
                    ...
                    //if (chat_session['last_row_id'] != undefined)
                    //	sessions_data[chat_id]['last_row_id'] = chat_session['last_row_id'];
                    //else
                    //	sessions_data[chat_id]['last_row_id'] = 0;
        ...
        if ((wpmudev_chat.pids['chat_session_message_update'] == '') && (wpmudev_chat.errors['chat_session_message_update'] < 10)) {
            ...
            if ((timers['users'] == 0) && (timers['meta'] == 0) && (timers['invites'] == 0) && (timers['messages'] == 0)) {
                ...
                //console.log('timers messages['+timers['messages']+'] invites['+timers['invites']+'] meta['+timers['meta']+'] users['+timers['users']+']');

                //console.log('sessions_data[%o]', sessions_data);
    ...
    chat_session_size_box: function () {
        ...
        for (var chat_id in wpmudev_chat.settings['sessions']) {
            ...
            //console.log('chat_session box_width['+parseInt(chat_session['box_width'])+'] box_height['+parseInt(chat_session['box_height'])+']');
            //console.log('chat_session box_width['+chat_session_box_width+'] box_height['+chat_session_box_height+']');

            //console.log('chat_session box_height['+parseInt(chat_session['box_height'])+'] w/shadow['+chat_session_box_height+']');
            ...
            if (chat_session['box_width_mobile_adjust'] == 'window') {
                if (chat_session_box_width > wpmudev_chat_localized['settings']['screen_width']) {
                    ...
                    //console.log('setting width: ['+wpmudev_chat_localized['settings']['screen_width']+'] (screen)');

                    //console.log('box_position_adjust_mobile['+chat_session['box_position_adjust_mobile']+']');
                    if ((jQuery('div#wpmudev-chat-box-' + chat_session['id']).hasClass('wpmudev-chat-box-site')) && (chat_session['box_position_adjust_mobile'] == 'enabled')) {
                        jQuery('#wpmudev-chat-box-' + chat_id).css('position', 'relative');

                        jQuery('#wpmudev-chat-box-' + chat_id).css('margin', '0');
                    ...
                    //console.log('setting width: ['+chat_session['box_width']+'] (session)');
...
//		jQuery('div.wpmudev-chat-box-site.wpmudev-chat-box-max div.wpmudev-chat-module-users-list').show();
        ...
        for (var chat_id in wpmudev_chat.settings['sessions']) {
            ...
            //console.log('chat_box_width['+chat_box_width+']');
            //console.log('chat_box_menu_width['+chat_box_menu_width+']');
    ...
    chat_session_handle_send_button: function (chat_id) {
        var chat_session = wpmudev_chat.chat_session_get_session_by_id(chat_id);
//		if ((chat_session['box_send_button_enable'] == 'enabled') || ((chat_session['box_send_button_enable'] == 'mobile_only') && (wpmudev_chat_localized['settings']['wp_is_mobile'] == true))) {
    ...
    chat_session_enqueue_message: function (message, chat_session) {
        ...
        if (message.length) {
            ...
            //console.log('adding idx['+microtime_timestamp+'] message['+message+'] chat_id['+chat_session.id+']');
        }

        //console.log('send_data[%o]', wpmudev_chat.send_data);
        wpmudev_chat.chat_session_messages_send();
    },
                    ...
                    if (reply_data != undefined) {
                        ...
                        if (reply_data['errorStatus'] != undefined) {
                            if (reply_data['errorStatus'] == true) {
                                ...
                                //console.log('reply chat_messages[%o]', reply_data['chat_messages']);
                                for (var chat_id in reply_data['chat_messages']) {
                                    ...
                                    for (var idx in chat_messages) {
                                        ...
                                        if ((chat_messages[idx] == true) && (wpmudev_chat.send_data[chat_id][idx] != undefined)) {
                                            ...
                                            //console.log('idx['+idx+']['+index_item+']');
                                            //console.log('removing idx['+idx+'] message['+message+'] chat_id['+chat_session.id+']');
...
//		}			
    ...
    chat_session_box_actions: function (chat_id) {
        ...
        console.log('chat_session_box_actions: adding chat_id:' + chat_id);
...
// The following is not longer used but produces a popup window for Teitter Auth. Using the above is a redirect and cleaner. 
    ...
    chat_privite_invite_click: function () {
        ...
        if (jQuery('.wpmudev-chat-button.wpmudev-chat-user-invite').length) {
            jQuery('.wpmudev-chat-button.wpmudev-chat-user-invite').click(function (event) {
                event.preventDefault();
                var user_hash = jQuery(this).attr('rel');
                user_hash = user_hash ? user_hash : '';
                if (user_hash != '') {
