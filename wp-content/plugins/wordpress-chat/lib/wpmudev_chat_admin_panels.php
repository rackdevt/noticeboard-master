		...
		function chat_settings_panel_site() {
			...
			<div id="wpmudev-chat-wrap" class="wrap wpmudev-chat-wrap-settings-page">
				...
				<form method="post" id="wpmudev-chat-settings-form" action="?page=<?php echo $_GET['page']; ?>">
					...
					<?php if (is_multisite()) {
						?><p><?php _e('Under Multisite the bottom corner chat can be enabled at the Netowrk. When this happens t will replace the local site bottom corner chat.', $wpmudev_chat->translation_domain); ?></p><?php
					} ?>
					<div id="chat_tab_pane" class="chat_tab_pane">
						<ul>
		...
		function chat_settings_panel_session_logs() {
			...
			if (isset($_GET['message'])) {
				...
				if (isset($wpmudev_chat->_admin_notice_messages[$message_idx])) {
					?><div id='chat-warning' class='updated fade'><p><?php echo $wpmudev_chat->_admin_notice_messages[$message_idx]; ?></p></div><?php
				}
			}
			if ((isset($_GET['laction'])) && ($_GET['laction'] == "show")) {
				...
				<div id="wpmudev-chat-messages-listing-panel" class="wrap wpmudev-chat-wrap wpmudev-chat-wrap-settings-page">
					...
					<p><a href="admin.php?page=chat_session_logs"><?php _e('Return to Logs', $wpmudev_chat->translation_domain); ?></a></p>
					<?php
						...
						if ((!empty($chat_session_transient)) && (is_array($chat_session_transient))) {
							...
							//echo "chat_session_transient_after<pre>"; print_r($chat_session_transient_after); echo "</pre>";

							echo $wpmudev_chat->process_chat_shortcode($chat_session_transient);

							//echo "chat_sessions<pre>"; print_r($wpmudev_chat->chat_sessions); echo "</pre>";
						}
				...
				<div id="wpmudev-chat-messages-listing-panel" class="wrap wpmudev-chat-wrap wpmudev-chat-wrap-settings-page">
					...
					<?php
						...
						if ((isset($wpmudev_chat->chat_log_list_table->log_item->deleted)) && ($wpmudev_chat->chat_log_list_table->log_item->deleted == 'yes')) {
							?><div id='chat-error' class='error fade'><p><?php _e('This entire Chat Session is marked as hidden. It will not be show on public logs display. You may still hide/unhide or delete individual messages below.', $wpmudev_chat->translation_domain); ?></p></div><?php
						}
