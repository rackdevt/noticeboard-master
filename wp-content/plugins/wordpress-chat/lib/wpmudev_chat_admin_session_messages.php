...
if (!class_exists('WPMUDEVChat_Session_Messages_Table')) {
	class WPMUDEVChat_Session_Messages_Table extends WP_List_Table {
		...
		function check_table_filters() {
			...
			if (is_multisite()) {
				if (strncasecmp($this->filters['chat_id'], 'private-', strlen('private-')) === 0) $this->filters['blog_id'] = 0;
		...
		function show_filters_chat_status() {
			...
			<select name="status" id="status">
				...
				<option <?php if ('archived' == $this->filters['status']) { echo ' selected="selected" '; } ?>
				...
				<select name="users" id="users">
					...
					<?php
						foreach($this->user_names as $user_name) {
							?><option <?php if ($user_name == $this->filters['users']) { echo ' selected="selected" '; } ?>
